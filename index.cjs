...
const dataModules=fs.readFileSync('modules.json', 'utf8');
var modules=JSON.parse(dataModules)

const btna='[ikikrepus\' bot]';

const commands = [
	new SlashCommandBuilder()
		.setName('help')
		.setDescription('Affiche l\'aide du bot')
		.setDefaultMemberPermissions(PermissionFlagsBits.Administrator), // uniquement les admins peuvent voir ça
	new SlashCommandBuilder()
		.setName('setup-embed')
		.setDescription('Affiche l\'embed de tickets')
		.setDefaultMemberPermissions(PermissionFlagsBits.Administrator), // pareil
	new SlashCommandBuilder()
		.setName('setup-categorie')
	...
	new SlashCommandBuilder()
		...
		.addChannelOption(option=>
			option.setName('salon')
			      ...
			      .setRequired(true)),
	new SlashCommandBuilder()
		.setName('setup-modulescreation')
		.setDescription('Indiquer au bot quels sont les modules à utiliser pour créer des tickets')
		.setDefaultMemberPermissions(PermissionFlagsBits.Administrator) // pareil
...
client.on('interactionCreate', async interaction=>{
	...
	await interaction.deferReply({ ephemeral: !((interaction.commandName==='help')) });
	const user=await interaction.guild.members.cache.get(interaction.user.id);
	if(!user.permissions.has(PermissionFlagsBits.Administrator)){
		interaction.editReply('<:warning:1386605398735458335> Permissions manquantes');
		return;
	}
	if(interaction.commandName==='help'){
		...
		const embed=new EmbedBuilder()
			.setColor(0xff7523)
			.setTitle(' ')
			.setDescription(`## Bienvenue dans le menu d'aide de <@1222311549478371460> !\n### Commandes disponibles :\n- \`/help\` : Ce que tu vois en ce moment !\n- \`/setup-categorie\` : Définit la catégorie à utiliser pour ranger les tickets\n- \`/setup-embed\` : Affiche l'embed en message non-éphémère (c'est-à-dire à la vue de tout le monde)\n- \`/setup-staffrole\` : Indique au bot le rôle du staff sur ce serveur`)
		interaction.editReply({
			embeds: [embed]
		})
	}else if(interaction.commandName==='setup-categorie'){
		console.log(`${btna} cmd setup-categorie envoyée par ${interaction.user.tag} id ${interaction.user.id}`);
	...
	}else if(interaction.commandName==='setup-embed'){
		console.log(`${btna} cmd setup-embed envoyée par ${interaction.user.tag} id ${interaction.user.id}`);
		const embed=new EmbedBuilder()
			.setTitle(' ')
		...
		const listeDeroulante=new ActionRowBuilder()
			.setComponents(
				new StringSelectMenuBuilder().setCustomId('ouvrir_ticket').setOptions([
					{ label: 'Postuler', value: 'ouvrirTicketPrePostuler', emoji: '<:t1:1386605361347428414>' },
					{ label: 'Aide', value: 'ouvrirTicketAide', emoji: '<:t2:1386605324936675368>' },
					{ label: 'Partenariat', value: 'ouvrirTicketPrePrePartenariat' , emoji: '<:t3:1386605261027934289>' },
					{ label: 'Autre', value: 'ouvrirTicketAutre', emoji: '<a:t4:1386605230107394131>' }
				])
			);
	...
	}else if(interaction.commandName==='setup-staffrole'){
...
client.on('interactionCreate', async interaction=>{
	...
	if(interaction.customId==='ouvrir_ticket'){
		...
		if(interaction.values[0].substr(12)==='PrePostuler'){
			...
			const champPkVous=new TextInputBuilder()
				.setCustomId('pkvous')
				.setLabel('Pourquoi vous ?')
				.setStyle(TextInputStyle.Paragraph)
				.setRequired(true)
				.setPlaceholder('Décrivez vos qualités dans ce domaine...');
			const champMotivations=new TextInputBuilder()
				.setCustomId('motivations')
				.setLabel('Quelles sont vos motivations ?')
				.setStyle(TextInputStyle.Short)
				.setRequired(true)
				.setPlaceholder('6h/jour');
			...
			const thirdActionRow=new ActionRowBuilder()
				.setComponents(champPkVous);
			const fourthActionRow=new ModalBuilder()
				.setComponents(champMotivations);
			const modal=new ModalBuilder()
  ...
  if(interaction.customId==='traiteTicket'){
    ...
    if(targetUser.permissions.has(PermissionFlagsBits.Administrator)){
      if(targetUser.id===tickets[guildId][channelId].owner){
        interaction.editReply({content:'<:warning:1386605398735458335> Vous ne pouvez pas traiter ce ticket car c\'est vous qui l\'avez ouvert !',ephemeral:true});
        return;
      }
      const embed=new EmbedBuilder()
        .setTitle(' ')
      ...
      interaction.editReply({ephemeral:true,content:'<:warning:1386605398735458335> Vous ne pouvez pas effectuer cette action administrateur !'});
    }
    ...
    if(previousAuthorId===interaction.user.id){
      ...
      await interaction.channel.send({content:'Export HTML généré :',files:[{attachment:Buffer.from(htmlRender),name:`${targetTicket.name}.html`}]});
      setTimeout(async()=>{delete tickets[interaction.guild.id][targetTicket.id];await targetTicket.delete();fs.writeFileSync('tickets.json',JSON.stringify(tickets,null,4),'utf8');},5000);
    }else{
      interaction.editReply(`<@${interaction.user.id}>, vous n'êtes pas l'auteur de cette action <:warning:1386605398735458335> !`);
    }
...
client.on('interactionCreate', async interaction=>{
	...
	if(interaction.customId==='ouvrirTicketPostuler'){
		...
		const pkvous=interaction.fields.getTextInputValue('pkvous');
		...
		const ghostPing=await thread.send(`${roles[interaction.guild.id]?`<@&${roles[interaction.guild.id]}>`:''}`);
		await ghostPing.delete();
		const text=new TextDisplayBuilder().setContent('<a:loading:1386605156791095448> Création du poll en cours...');
...
a=(msgcontent=>{
  ...
  return returnDescription;
	...
	console.log(objectRender.content)
	if((message?.embeds[0]?.description===`## Le ticket est inactif...\nLe ticket semble inactif depuis peu, voulez-vous le supprimer ?`)&&(message.author.id='1222311549478371460'))return;
	queue[message.guild.id]=queue[message.guild.id]||[];
	queue[message.guild.id].push(message.id);
	setTimeout(()=>{
		if(queue[message.guild.id].length===1){
			...
			try{
				message.channel.send({
					content: `<@${roles[message.guild.id]}>`,
					embeds: [embed],
...
a=(msgcontent=>{
  ...
  return returnDescription;
	...
	if((message?.embeds[0]?.description===`## Le ticket est inactif...\nLe ticket semble inactif depuis peu, voulez-vous le supprimer ?`)&&(message.author.id='1222311549478371460'))return;
	queue[message.guild.id]=queue[message.guild.id]||[];
	queue[message.guild.id].push(message.id+'edited');
	setTimeout(()=>{
		if(queue[message.guild.id].length===1){
			const embed=new EmbedBuilder()
				.setTitle(' ')
...
client.on('messageCreate', async message=>{
	if(!(message.channel.isThread()||message.channel.isTextBased()))return;
...
client.on('interactionCreate', async interaction=>{
	if((interaction.commandName==='setup-modulescreation')||(interaction.customId==='setup-modulescreation')){
		modules[interaction.guild.id]=modules[interaction.guild.id]||[];
		const text1=new TextInputBuilder().setContent(`Choisissez les modules de création de tickets à activer pour ce serveur :`);
		const select=new StringSelectMenuBuilder()
			.setCustomId('modules-select')
			.setPlaceholder('Aucun module sélectionné')
			.addOptions([{
					name: 'Postuler',
					id: 'postuler',
					emoji: '<:t1:1386605361347428414>'
				},
				{
					name: 'Aide',
					id: 'aide',
					emoji: '<:t2:1386605324936675368>'
				},
				{
					name: 'Partenariat',
					id: 'partenariat',
					emoji: '<:t3:1386605261027934289>'
				},
				{
					name: 'IA',
					id: 'ia',
					emoji: '<:orbs:1410998778189709312>'
				},
				{
					name: 'Autre',
					id: 'autre',
					emoji: '<a:t4:1386605230107394131>'
				}].map(module=>({
					label: module.name,
					value: module.id
				}))
			);
		const text2=new TextDisplayBuilder().setContent(`Voici ce que vous avez choisi comme modules d'ouverture de tickets :${(()=>{ let str='';modules[interaction.guild.id].forEach(module=>{ str+=`\n* **${module}**` }); return str||'\n**Rien de choisi**' })()}\n*Pour déselectionner appuyer sur un élément*\n${(()=>{ if(interaction.values.length){ return `> **Action effectuée** :\n> ` }})()}`)
	}
})
